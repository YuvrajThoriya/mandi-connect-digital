  ...
  const fetchAuctions = async () => {
    try {
      ...
      if (!auctionsData || !Array.isArray(auctionsData)) {
        console.log('No auctions data found');
        setAuctions([]);
        setCategories([]);
        setLocations([]);
        setIsLoading(false);
      ...
      const transformedAuctions: AuctionProduct[] = auctionsData
        .filter(auction => {
          // Ensure both product and profile data exists
          if (!auction.products || !auction.profiles) {
            console.warn('Missing product or profile data for auction:', auction.id);
            return false;
          }
          return true;
        })
        .map(auction => {
          // TypeScript type assertion for nested objects
          const product = (auction as any).products as NonNullable<DatabaseAuction['products']>;
          const profile = (auction as any).profiles as NonNullable<DatabaseAuction['profiles']>;
      ...
      toast({
        title: "Error",
        description: error instanceof Error ? error.message : "Failed to load auctions. Please try again.",
        variant: "destructive"
      });
    } finally {
      setIsLoading(false);
